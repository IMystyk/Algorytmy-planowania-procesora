def exp(processess, arrivalTime, burstTime, priorities):
    # Realizes expiring priority algorithm
    n = len(processess)
    ...
    while True:
        ...
        # Check if a process that was being executed before is still in privileged
        if prevProcess == -1:
            prevProcess = nextProcess
        if prevProcess in privilegedProcesses:
            pass
        else:
